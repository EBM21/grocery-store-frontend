...
import { 
  ...
  Upload, ShoppingBag, Truck, DollarSign, Check, X, Clock 
} from "lucide-react";

export default function AdminPage() {
  
  // --- YAHAN APNA RENDER LINK DALEIN ---
  const API_URL = "https://grocery-store-backend-wxpw.onrender.com"; 
  ...
  const fetchData = async () => {
    try {
        // --- SARY LINKS UPDATE KAR DIYE HAIN ---
  ...
  // --- HANDLERS (UPDATED LINKS) ---
  
  const handlePromoUpdate = async (e) => {
    e.preventDefault();
    try {
        await fetch(`${API_URL}/promo`, {
            method: "PUT",
  ...
  const handleSliderSubmit = async (e) => {
    ...
    await fetch(`${API_URL}/sliders`, { method: "POST", body: formData });
    setSliderImage(null); fetchData();
  };

  const handleDeleteSlider = async (id) => {
    if(confirm("Delete Slider?")) { await fetch(`${API_URL}/sliders/${id}`, { method: "DELETE" }); fetchData(); }
  };
  ...
  const handleCategorySubmit = async (e) => {
    ...
    await fetch(`${API_URL}/categories`, { method: "POST", body: formData });
    setCategoryData({ name: "", discount_percent: "0" }); setCategoryImage(null); fetchData();
  };

  const handleDeleteCategory = async (id) => {
    if(confirm("Delete Category?")) { await fetch(`${API_URL}/categories/${id}`, { method: "DELETE" }); fetchData(); }
  };
  ...
  const handleDeleteProduct = async (id) => {
    if(confirm("Delete Product?")) { await fetch(`${API_URL}/products/${id}`, { method: "DELETE" }); fetchData(); }
  };
  ...
  return (
    <div className="admin-wrapper">
        ...
        <div className="content-card orders-section">
            ...
            <div className="table-responsive">
                <table className="modern-table">
                    <thead><tr><th>ID</th><th>Customer</th><th style={{width:'25%'}}>Address</th><th>Items</th><th>Total</th><th>Status</th><th>Action</th></tr></thead>
                    <tbody>
                        {orders.map(order => (
                            <tr key={order.id}>
                                <td className="font-bold">#{order.id}</td>
                                <td><div className="customer-name">{order.customer_name}</div><div className="customer-phone">{order.phone}</div></td>
                                <td><div className="address-box"><span className="city-tag">{order.city}</span><p>{order.address}</p></div></td>
                                <td className="items-cell">{formatItems(order.items)}</td>
                                <td className="price-cell">Rs. {order.total_amount}</td>
                                <td>
                                    <select value={order.status || 'Pending'} onChange={(e) => handleOrderStatus(order.id, e.target.value)} className={`status-select ${order.status === 'Completed' ? 'completed' : 'pending'}`}>
                                        <option value="Pending">Pending</option><option value="Completed">Completed</option><option value="Cancelled">Cancelled</option>
                                    </select>
                                </td>
                                <td><button onClick={() => handleDeleteOrder(order.id)} className="icon-btn delete"><Trash2 size={18} /></button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
        ...
        <div className="grid-2-1">
          <div className="left-column">
            ...
            <div className="content-card">
                <div className="card-header"><h2><Plus size={20} /> Add New Product</h2></div>
                <form onSubmit={handleProductSubmit} className="product-form">
                    ...
                    <div className="form-row full-width">
                        <textarea name="description" placeholder="Product Description..." value={productData.description} onChange={e=>setProductData({...productData, description:e.target.value})} className="input-field textarea" />
                    </div>
                    <div className="file-upload-wrapper">
                        <label className="file-label"><Upload size={16} /> Upload Image File (Optional)<input type="file" onChange={e=>setProductImage(e.target.files[0])} /></label>
                        {productImage && <span className="file-name">{productImage.name}</span>}
                    </div>
                    <button type="submit" disabled={loading} className="submit-btn full-width">{loading ? "Adding..." : "Add Product"}</button>
                </form>
            </div>
            
             {/* PRODUCT LIST */}
             <div className="content-card">
                <div className="card-header"><h2><Package size={20} /> Inventory</h2></div>
                <div className="table-responsive" style={{maxHeight: '400px'}}>
                    <table className="modern-table">
                        <thead><tr><th>Img</th><th>Name</th><th>Price</th><th>Action</th></tr></thead>
                        <tbody>{products.map(p => (<tr key={p.id}><td><img src={p.image_url} className="product-thumb" /></td><td>{p.title}</td><td className="price-text">Rs. {p.price}</td><td><button onClick={()=>handleDeleteProduct(p.id)} className="icon-btn delete"><Trash2 size={16}/></button></td></tr>))}</tbody>
                    </table>
                </div>
            </div>
          </div>
          ...
          <div className="right-column">
             ...
             <div className="content-card" style={{border: '2px solid #FFD700'}}>
                <div className="card-header"><h2><Clock size={20} /> Promo Bar</h2></div>
                <div className="mini-form">
                    <label style={{display:'flex', alignItems:'center', gap:'10px', fontSize:'14px', fontWeight:'600'}}>
                        <input type="checkbox" checked={promoData.is_active} onChange={e => setPromoData({ ...promoData, is_active: e.target.checked })} /> Show Promo Bar
                    </label>
                    <input placeholder="Message" value={promoData.message} onChange={e => setPromoData({ ...promoData, message: e.target.value })} className="input-field" />
                    <input type="datetime-local" value={promoData.end_time ? new Date(promoData.end_time).toISOString().slice(0, 16) : ""} onChange={e => setPromoData({ ...promoData, end_time: e.target.value })} className="input-field" />
             ...
             {/* SLIDER MANAGER */}
             <div className="content-card">
                <div className="card-header"><h2><ImageIcon size={20} /> Sliders</h2></div>
                <div className="mini-form">
                    <div className="file-upload-wrapper"><label className="file-label"><Upload size={16} /> Banner<input type="file" onChange={e=>setSliderImage(e.target.files[0])} /></label></div>
                    <button onClick={handleSliderSubmit} className="submit-btn">Upload</button>
                </div>
                <div className="slider-grid">{sliders.map(s => (<div key={s.id} className="slider-item"><img src={s.image_url} /><button onClick={()=>handleDeleteSlider(s.id)}><X size={14}/></button></div>))}</div>
             </div>

             {/* CATEGORY MANAGER */}
             <div className="content-card">
                <div className="card-header"><h2><Layers size={20} /> Categories</h2></div>
                <div className="mini-form">
                    <input placeholder="Name" value={categoryData.name} onChange={e=>setCategoryData({...categoryData, name:e.target.value})} className="input-field" />
                    <div className="file-upload-wrapper"><label className="file-label"><Upload size={16} /> Icon<input type="file" onChange={e=>setCategoryImage(e.target.files[0])} /></label></div>
                    <button onClick={handleCategorySubmit} className="submit-btn">Add Category</button>
                </div>
                <div className="category-list">{categories.map(c => (<div key={c.id} className="category-row"><span>{c.name}</span><button onClick={()=>handleDeleteCategory(c.id)} className="icon-btn delete small"><Trash2 size={14}/></button></div>))}</div>
             </div>
          </div>
        </div>
      </div>
      <style jsx>{`
        ...
        .product-form .form-row.full-width { grid-template-columns: 1fr; }
